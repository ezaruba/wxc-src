...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #WXCOINTEST2\r");
            Send(hSocket, "WHO #WXCOINTEST2\r");
        } else {
            // randomly join #WXCOIN00-#WXCOIN05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #WXCOIN%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #WXCOIN%02d\r", channel_number).c_str());
        }
