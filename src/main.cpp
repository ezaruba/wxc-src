...
CBigNum bnProofOfWorkLimit(~uint256(0) >> 17); // "standard" scrypt target limit for proof of work, results with 0,000244140625 proof-of-work difficulty
CBigNum bnProofOfStakeLegacyLimit(~uint256(0) >> 17); // proof of stake target limit from block #15000 and until 20 June 2013, results with 0,00390625 proof of stake difficulty
CBigNum bnProofOfStakeLimit(~uint256(0) >> 17); // proof of stake target limit since 20 June 2013, equal to 0.03125  proof of stake difficulty
CBigNum bnProofOfStakeHardLimit(~uint256(0) >> 17); // disabled temporarily, will be used in the future to fix minimal proof of stake difficulty at 0.25
...
const string strMessageMagic = "WXCOIN Signed Message:\n";
...
{
    int64_t nSubsidy = 	0.052 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 237500000;
    if (halvings >= 64)
        return nFees;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in WXCOIN
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // WXCOIN: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WXCOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "GXC GENESIS";
        CTransaction txNew;
        txNew.nTime = 1521996558;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1521996558;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 440382; // !fTestNet ? 1575379 : 46534;
