...
Alternatively, install Qt Creator and open the `WXCOIN-qt.pro` file.

An executable named `WXCOIN-qt` will be built.
...
A warning for people using the *static binary* version of WXCOIN on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of WXCOIN is linked against libdb5.3.
...
installed causes WXCOIN-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
